...
// Order shipped email — fires when admin transitions an order to
// "shipped" status (typically with a tracking number). Mirrors the
// styling of orderConfirmationHtml so customers see consistent branding.
export function orderShippedHtml(order) {
  const trackingBlock = order.tracking_number
    ? `<div style="margin:24px 0;padding:18px;background:#f5f3ee;border:1px solid #e5e2da">
         <div style="font-size:11px;color:#6b7689;letter-spacing:.18em;text-transform:uppercase;font-weight:600;margin-bottom:6px">Tracking</div>
         <div style="font-family:monospace;font-size:15px;color:#0c1422;font-weight:600">${escapeHtml(order.tracking_number)}</div>
       </div>`
    : `<p style="margin:0 0 24px;color:#6b7689;font-size:14px">Tracking info will follow shortly.</p>`;
...
<html><body style="font-family:-apple-system,sans-serif;background:#fafaf6;padding:32px;margin:0">
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e5e5e5;border-radius:12px;overflow:hidden">
    ...
    <div style="padding:40px 32px">
      <h1 style="margin:0 0 8px;font-size:24px;color:#0c1422">Your order is on the way.</h1>
      <p style="margin:0 0 24px;color:#6b7689;font-size:15px">
        Order #${escapeHtml(order.id.slice(0, 8).toUpperCase())} just shipped.
      </p>
      ${trackingBlock}
      <p style="margin:0;color:#6b7689;font-size:13px;line-height:1.6">
        Questions? Just reply to this email.
      </p>
    </div>
    <div style="padding:24px 32px;background:#f5f3ee;color:#6b7689;font-size:12px;text-align:center">
      Thank you for choosing TAGLINE.
    </div>
  </div>
</body></html>
  `;
