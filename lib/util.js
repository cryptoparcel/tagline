...
import { getSupabaseAdmin, getUserFromRequest } from './supabase.js';
...
// Per-user admin check: looks up the Bearer-token user's profiles row and
// returns true iff is_admin = true. Cheap to call (the auth token → user
// lookup is cached in lib/supabase.js).
//
// Used together with isAdmin(req) so an admin can authenticate either way:
//   1. ADMIN_API_KEY in X-Admin-Key header (legacy, key-based)
//   2. Signed in to the site as a profile with is_admin = true
//
// The combined helper requireAdmin(req, res) handles the precedence and
// 401 response — most endpoints should use that.
...
// Combined admin check — accept either the API key or a signed-in admin.
// Returns true iff the request is authorized. Async because the user
// lookup involves a Supabase round trip on cache miss.
export async function requireAdmin(req, res) {
  if (isAdmin(req)) return true;
  if (await isAdminUser(req)) return true;
  json(res, 401, { ok: false, error: 'Admin access required.' });
  return false;
}
